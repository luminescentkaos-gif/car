...
uint32_t SSD1306_Init(void)
{
    ...
    result = I2cInit(WIFI_IOT_I2C_IDX_0, SSD1306_I2C_SPEED);
    if (result != 0)
    {
    ...
    result = I2cSetBaudrate(WIFI_IOT_I2C_IDX_0, SSD1306_I2C_SPEED);
    if (result != 0)
    {
      printf("I2C SSD1306 baudrate status is 0x%x!!!\n", result);
      return result;
    }
