...
#define RAINBOW_NUM (sizeof(rainbow_colors)/sizeof(rainbow_colors[0]))
	
void L1_runingled(void)    //跑马灯
{
	u8 i,j;
/*流光*/	
	for(j = 1; j < 7; j++)
    {
        for(i = 1; i < 7; i++)
        {
            if(i == j)
            {
                u8 idx = (i - 1) % RAINBOW_NUM;
                L_ws2812_rgb(i, rainbow_colors[idx][0], rainbow_colors[idx][1], rainbow_colors[idx][2]);
                R_ws2812_rgb(i, rainbow_colors[idx][0], rainbow_colors[idx][1], rainbow_colors[idx][2]);
            }
            else
            {
                L_ws2812_rgb(i, WS_DARK);
