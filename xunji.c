...
#define JUNCTION_COOLDOWN_CYCLES 30
...
static void line_follow_task(void)
{
    ...
    uint8_t in_junction = 0;
    uint8_t junction_cooldown = 0;
    ...
    while (1) {
        ...
        if (++print_count >= 20) {
            print_count = 0;
            printf("IR raw: L=%u R=%u, line: L=%u R=%u, junction=%u\n",
                left_level, right_level, left_on_line, right_on_line, junction_count);
        }
        ...
        if (!left_on_line && !right_on_line) {
            if (in_junction) {
                in_junction = 0;
                junction_cooldown = JUNCTION_COOLDOWN_CYCLES;
            }
            ...
            /* 双黑：前两次当岔路，按固定方向转向 */
            if (junction_count < 2) {
                if (!in_junction && junction_cooldown == 0) {
                    junction_count++;
                    in_junction = 1;
                    printf("Junction %d detected\n", junction_count);
                }
                if (junction_count == 1) {
                    motor_control(-JUNCTION_SPEED, JUNCTION_SPEED);   /* 左转 */
                } else {
                    motor_control(JUNCTION_SPEED, -JUNCTION_SPEED);   /* 右转 */
                }
            } else {
                /* 已过两个岔路，双黑由上面的终点检测处理，这里兜底直行 */
                motor_control(BASE_SPEED, BASE_SPEED);
            }
        }
